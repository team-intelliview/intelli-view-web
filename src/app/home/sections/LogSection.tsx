...
import { MODAL_PATH, PATH, REQUEST_OPTION } from '@/constants';
...
const LogSection = ({ type }: LogSectionProps) => {
  ...
  const handleProcessClick = () => {
    ...
    type === REQUEST_OPTION.INTERVIEW
      ? router.push(MODAL_PATH.INTERVIEW_TYPE)
      : router.push(PATH.JOB);
  };

  return (
    <Card className="relative flex w-full flex-col bg-white px-[24px] pb-[32px]">
      ...
      <div className="pointer-events-none absolute bottom-0 left-0 h-[120px] w-full rounded-b-[20px] bg-gradient-to-t from-white to-transparent" />
    </Card>
  );
};
